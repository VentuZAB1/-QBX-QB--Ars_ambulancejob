...
-- Simple kill command for testing (anyone can use)
RegisterCommand('testkill', function(source, args)
    ...
    TriggerClientEvent('ars_ambulancejob:killPlayer', targetId)
end)

-- Kill command for testing (with permissions)
RegisterCommand('kill', function(source, args)
    local QBCore = exports['qb-core']:GetCoreObject()
    local src = source

    -- For QBOX, we'll check if the player has staff permissions
    if src > 0 then -- Not console
        local hasPermission = QBCore.Functions.HasPermission(src, 'admin') -- QBOX uses this format
    ...
    -- Notify the killer
    if src ~= 0 then -- Don't send notification to console
        TriggerClientEvent('QBCore:Notify', src, 'Player has been killed', 'success')
    end
end)
